...
- `/api/import` (session-gated, POST `{url, mode}`, `maxDuration` 60s in vercel.json). **Pages are loaded in headless Chromium first** (`loadPage` -> `renderPage`: `puppeteer-core`, on Vercel `@sparticuz/chromium` via dynamic `import()` because the package is ESM-only, locally an installed Chrome/Edge or `CHROME_PATH`; images/fonts/media requests are blocked for speed, the page is auto-scrolled so lazy content appears, CSSOM-inserted rules are folded back into `<style>` text and `currentSrc` pinned before the DOM is serialized). Without a browser, or when rendering fails, it falls back to the raw HTML and the response says so (`rendered`). `IMPORT_NO_RENDER=1` forces the raw path. Each whole-site page is its own request, so a rendered page (10 to 20s) always fits the limit; the response size stays under Vercel's 4.5MB cap because stylesheets already sent are omitted (`have`). `mode:'exact'` returns the page as-is: every
