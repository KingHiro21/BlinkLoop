...
| `api/` | Serverless functions (CommonJS — do NOT add `"type":"module"` to package.json). Besides auth/chat: `lead.js` + `leads.js` (contact forms and the admin Leads panel), `import.js` (website importer), `publish.js` (Vercel API publishing), `preview.js` (private preview links), `ai.js` (copy rewriting), `upload.js` (Blob uploads) |
